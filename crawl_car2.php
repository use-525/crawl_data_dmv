...
function pushDataExam($NumberQuestion, $Name, $conn, $StateCode, $numberTest)
{
    ...
    for ($numberExam = 1; $numberExam <= $numberTest; $numberExam++) {
        ...
        if ($conn->query($sql)) {
            ...
            pushDataQuestion($Name, $NumberQuestion, $numberExam, $conn, $last_id,$StateID);
        } else {
            echo "Error: " . $sql . "<br>" . $conn->error;
        }
    }
...
function pushDataQuestion($Name, $NumberQuestion, $numberExam, $conn, $last_id,$StateID)
{
    ...
    while ($y < $NumberQuestion) {
        ...
        foreach ($all_question_row as $question_row) {
            ...
            foreach ($question_row->find('.answer-image-holder') as $element) {
                ...
                if (isset($image->src)) {
                    $imageSrc = $image->src;
                    $nameImage = preg_replace('/https:\/\/dmv-practice-test.com\/storage\/images-car\//', '', $imageSrc);
                    file_put_contents('Image_Question/'.'question_car_'.$nameImage, file_get_contents($imageSrc));
                     $QuestionImageName = 'question_car_'.$nameImage;
                } else {
                    $QuestionImageName = '';
                }
            ...
            $sql1 = "INSERT INTO Question (`StateID`, `GroupID`, `QuestionText`,`QuestionImageName`,
        Answer1,Answer2,Answer3,Answer4,AnswerCorrect,`Explain`,Difficult,QuestionID)
        VALUES ($StateID,1,'$question_text','$QuestionImageName','$answer_text[0]','$answer_text[1]','$answer_text[2]','$answer_text[3]','$data_value','$Explain',1,'$data_id')";
            if ($conn->query($sql1)) {
                $question_id = $data_id;
            } else {
