...
foreach ($fakedatastate as $key => $state) {
    ...
    if ($conn->query($sql)) {
        $html = file_get_html('https://dmv-practice-test.com/' . $Name . '/motorcycle/practice-test-1');
        foreach ($html->find('#series-content-holder') as $series_content_holder) {
            ...
            VALUES ('$conn->insert_id', '$NumberQuestion' , '$MiniumAnswerCorrect','motorcycle')";
            if ($conn->query($sql)) {
...
function pushDataQuestion($Name, $NumberQuestion, $numberExam, $conn, $last_id)
{
    ...
    while ($y < $NumberQuestion) {
        $url = 'https://dmv-practice-test.com/' . $Name . '/motorcycle/practice-test-' . $numberExam . '/' . $x;
